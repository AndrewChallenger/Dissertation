...
#hyperparameters hyperpriors of lambda and gamma
a <- 2
b <- 2
c <- 2
d <- 2
...
#Fitting the model
fit <- stan(file = 'pl_model1.stan', data = stan_data, iter = iters, warmup = burnin, seed = 26)

save(fit, file = 'pl_model1_fit')

sum <- rstan::summary(fit)$summary
save(sum, file = 'pl_model1_summary')

#Log marginal likelihood
log_lik <- bridge_sampler(fit)
save(log_lik, file = 'pl_model1_loglik')
